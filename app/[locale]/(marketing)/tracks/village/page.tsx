...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: t("tracks.village.title"), description: t("tracks.village.tagline") };
}

export default async function VillageTrackPage({ params }: Props) {
  const { locale } = await params;
  ...
  return (
    <>
      <Hero
        variant="image"
        imageUrl="/generated/hero/track-village.png"
        eyebrow={t("nav.tracks")}
        headline={t("tracks.village.title")}
        body={t("tracks.village.tagline")}
        topBadge={<AnimatedBadge variant="dark">{t("tracks.village.layout")}</AnimatedBadge>}
        primaryCta={
        ...
        <div className="section-y mx-auto max-w-7xl container-px">
          <Breadcrumb
            items={[
              ...
              { label: t("tracks.village.title") },
            ]}
          />
          <div className="mt-10 grid items-center gap-10 lg:grid-cols-2">
            <MotionReveal>
              <SectionHeader
                eyebrow="The village layout"
                heading={t("tracks.village.title")}
                subhead={t("tracks.village.tagline")}
                badge={<AnimatedBadge dotColor="orange">Technical & flowing</AnimatedBadge>}
              />
              <dl className="mt-8 grid grid-cols-2 gap-4">
                <Stat icon={<Ruler size={18} aria-hidden />} label="Length" value={t("tracks.village.length")} />
                <Stat icon={<Trophy size={18} aria-hidden />} label="Best for" value={t("tracks.village.best")} />
                <Stat icon={<Timer size={18} aria-hidden />} label="Lap record" value={t("tracks.village.lapRecord")} />
                <Stat icon={<MapPin size={18} aria-hidden />} label="Layout" value={t("tracks.village.layout")} />
              </dl>
            </MotionReveal>
